...
class Sinkhorn_controller():
    ...
    def causal_unconstrained_Sink(self, datas, radius, eps):
...
def main():
    ...
    [Phi_x, Phi_u, obj] = controller.causal_unconstrained_Sink(data_points, rho_bar**2, eps)
    [Phi_x_W, Phi_u_W, obj_W] = controller.causal_unconstrained_Wass(data_points, rho**2)
    # [_, P, _] = ct.dlqr(A, B, np.eye(A.shape[0]), np.eye(B.shape[1]))
